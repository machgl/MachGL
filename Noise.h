...
Mach::GL (Alpha) 
	...
	C++ implementation of Perlin Noise
	ORIGINAL IMPLEMENTATION OF IMPROVED NOISE COPYRIGHT 2002 KEN PERLIN
...
namespace MachGL {
	namespace Maths {
		...
		class Noise {
			...
			private:
				...
				std::vector<int> permutation  {
					...
					242,193,238,210,144,12,191,179,162,241,81,51,145,235,249,14,239,107,49,192,
			...
			public:
				Noise();
				Noise(const long& seed);
				double getNoise(double x, double y, double z);
		};

		Noise::Noise() {

			p.insert(p.begin(), permutation.begin(), permutation.end());
			p.insert(p.end(), p.begin(), p.end());

		}

		Noise::Noise(const long& seed) : m_seed(seed) { 
		...
		double Noise::fade(const double& t) {
			return t * t * t * (t * (t * 6 - 15) + 10);
		}

		double Noise::lerp(const double& t, const double& a, const double& b) {
			return a + t * (b - a);
		}

		double Noise::grad(const int& hash, const double& x, const double& y, const double& z) {
